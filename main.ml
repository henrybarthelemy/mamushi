...
let () =
  let speclist =
    [ ("-t", Arg.Set show_trace, "Display the trace of compilation");
      ...
      ("-no-typechecking", Arg.Set no_typechecking, "Leave out type checking");
      ]
  in
  Arg.parse speclist
    (fun name ->
    ...
    "Compiler options:";
  let sep = "\n=================\n" in
  match compile_file_to_string ~no_builtins:!no_builtins ~no_typechecking:!no_typechecking !alloc_strat !filename !filename with
  | Error (errs, trace) ->
      if !show_trace then
